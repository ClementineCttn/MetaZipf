...
shinyUI(fluidPage(
  ...
  navlistPanel(
    ...
    tabPanel("Literature Overview",
             fluidRow(column(6, h4("Choose the length of the top:")),
                             column(6,sliderInput("top", "TOP", min = 1, max=20, value = 5))),
            h4('The TOP journals where the estimations* are drawn from are:'),  
            tableOutput('topjournals'),
            '*Each reference count as one, irrespective of the number of estimations',   br(),
            h4('The TOP authors* providing estimations are:'), 
            tableOutput('topauthors'),
            '*the estimations published by the same author(s) in different publications are not added.',    br(),
            h4('The TOP countries for the diversity of results* are:'),
            tableOutput('topcountries'),
            '*measured by the coefficient of variation for countries with more than 5 estimations.',    br(),
            h4('The TOP maximum and minimum Alpha measured are:'),
            tableOutput('topextremes'),
